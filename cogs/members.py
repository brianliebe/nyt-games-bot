...
class MembersCog(commands.Cog, name="Normal Members Commands"):
    ...
    async def get_ranks(self, ctx, *args):
        ...
        for user_id in self.db.get_all_players():
            ...
            if len(intersection) > 0:
                stats.append(PlayerStats(user_id, valid_puzzles, self.db))
    ...
    async def on_message(self, message):
        ...
        if '\n' in message.content:
            ...
            if self.utils.is_wordle_submission(title):
                self.db.add_entry(str(message.author.id), title, content)
    ...
    async def view_entry(self, ctx, *args):
        ...
        if user_id in self.db.get_all_players():
            user_puzzles: list[PuzzleEntry] = self.db.get_entries_by_player(user_id, [])
    ...
    async def get_stats(self, ctx, *args):
        ...
        for i, user_id in enumerate(user_ids):
            ...
            player_stats = PlayerStats(user_id, puzzle_list, self.db)
            df.loc[i] = [
                self.utils.get_nickname(user_id),
        ...
        if len(user_ids) < 5:
            ...
            for i, user_id in enumerate(user_ids):
                ...
                entries: list[PuzzleEntry] = self.db.get_entries_by_player(user_id, [])
